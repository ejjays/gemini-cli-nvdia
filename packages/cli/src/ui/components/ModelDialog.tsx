...
export function ModelDialog({ onClose }: ModelDialogProps): React.JSX.Element {
  ...
  const manualOptions = useMemo(() => {
      ...
      options.push(
        {
          ...
          title: getDisplayString(DEEPSEEK_V4_FLASH, config ?? undefined),
          key: DEEPSEEK_V4_FLASH,
        },
        {
          value: DEEPSEEK_V4_PRO,
          title: getDisplayString(DEEPSEEK_V4_PRO, config ?? undefined),
          key: DEEPSEEK_V4_PRO,
        },
      );
    }
    ...
    if (!hasAccessToProModel) {
      // Filter out Gemini Pro models for free tier, but allow other providers' Pro models
      return options.filter(
        (option) =>
          !option.value.startsWith('gemini-') || !isProModel(option.value),
      );
    }
