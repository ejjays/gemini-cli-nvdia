...
export async function validateNonInteractiveAuth(
  ...
  try {
    ...
    // Prioritize NVIDIA auth if the model explicitly requires it and we have the key.
    const requestedModel = nonInteractiveConfig.getModel();
    const modelDef =
      nonInteractiveConfig.modelConfigService.getModelDefinition(
        requestedModel,
      );
    if (
      modelDef?.authType === (AuthType.NVIDIA_API as string) &&
      process.env['NVIDIA_API_KEY']
    ) {
      effectiveAuthType = AuthType.NVIDIA_API as AuthType;
    }
