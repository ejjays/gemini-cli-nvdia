...
export async function createContentGeneratorConfig(
  ...
  if (authType === AuthType.NVIDIA_API) {
    const nvidiaApiKey = apiKey || process.env['NVIDIA_API_KEY'];
    contentGeneratorConfig.apiKey = nvidiaApiKey;
    contentGeneratorConfig.vertexai = false;
    contentGeneratorConfig.baseUrl = baseUrl || 'https://integrate.api.nvidia.com/v1';
...
export async function createContentGenerator(
  ...
  const generator = await (async () => {
    ...
    if (
      ...
      config.authType === AuthType.NVIDIA_API
    ) {
      ...
      // Use OpenAI adapter for NVIDIA API
      if (config.authType === AuthType.NVIDIA_API) {
